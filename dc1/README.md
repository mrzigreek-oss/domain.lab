...
```powershell
Rename-Computer DC1
New-NetIPAddress -InterfaceAlias "Ethernet0" -IPAddress 172.1.1.2 -PrefixLength 24 -DefaultGateway 172.1.1.1
...
Import-GPO -BackupId "{C4402DE5-1BA2-47DD-B680-C8AF127DD3FB}" -Path "$gpoDir\Domain" -TargetName "LAPS - Конфигурация (Windows LAPS)" -CreateIfNeeded | Out-Null
```

## Не используем, в новых редакциях винды LAPS встроен в систему и легаси лапс через msi больше не нужен, сделал отдельную политику и забэкапил её последней
```powershell
...
```
В группы добавляю обе, но если это администратор юнита только в админы домена безопасности добавляем.
###### После развёртывания сервера безопасности добавить туда пользователя rootts 
```powershell
Invoke-Command -ComputerName "Trustcoverbd" -ScriptBlock {
